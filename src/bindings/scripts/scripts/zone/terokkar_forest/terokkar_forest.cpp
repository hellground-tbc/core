...
struct TRINITY_DLL_DECL quest_the_vengeful_harbringerAI : public ScriptedAI
{
...

    {
        ...
        visual_energy->MonsterMoveWithSpeed(portal_trigger->GetPositionX(),portal_trigger->GetPositionY(),portal_trigger->GetPositionZ(),1500);
    ...
    void UpdateAI(const uint32 diff)
    {
        if (visual_1_timer < diff)
        {
            if (Creature * visual_energy = GetClosestCreatureWithEntry(me, 21429, 30.0f))
            {
                ...
                Dranei_Guardian->MonsterMoveWithSpeed(Defender_Trigger->GetPositionX(),Defender_Trigger->GetPositionY(),Defender_Trigger->GetPositionZ(),6000); //we ned this to visual movement with delay
            }
        }
        else
            visual_1_timer-=diff;
