...
bool GossipHello_npc_spirit_of_olum(Player* player, Creature* _Creature)
{
    ...
    if(pInstance && (pInstance->GetData(DATA_SHADEOFAKAMAEVENT) >= DONE) && (pInstance->GetData(DATA_SUPREMUSEVENT) >= DONE) && (pInstance->GetData(DATA_HIGHWARLORDNAJENTUSEVENT) >= DONE))
