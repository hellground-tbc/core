...
bool MailDraft::prepareItems(Player* receiver)
{
    ...
    // can be empty
    mailLoot.FillLoot(m_mailTemplateId, LootTemplates_QuestMail, receiver, true, true);
...
void MailDraft::SendMailTo(MailReceiver const& receiver, MailSender const& sender, MailCheckMask checked, uint32 deliver_delay)
{
    ...
    if (sender.GetMailMessageType() == MAIL_AUCTION && m_items.empty() && !m_money)
        expire_delay = HOUR;
//    // mail from battlemaster (rewardmarks) should last only one day //TODO: uncomment when bgmgr will handle it
...
void Mail::prepareTemplateItems( Player* receiver )
{
    ...
    // can be empty
    mailLoot.FillLoot(mailTemplateId, LootTemplates_QuestMail, receiver, true, true);
