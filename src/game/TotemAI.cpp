...
TotemAI::UpdateAI(const uint32 /*diff*/)
{
        ...
        ( i_totem.IsFriendlyTo(victim) && victim != &i_totem ) || !victim->isVisibleForOrDetect(&i_totem,false) )
    {
        ...
        cell_lock->Visit(cell_lock, grid_object_checker,  *i_totem.GetMap(), i_totem, max_range);
        cell_lock->Visit(cell_lock, world_object_checker, *i_totem.GetMap(), i_totem, max_range);
