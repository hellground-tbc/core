...
void AuctionHouseMgr::SendAuctionWonMail(AuctionEntry *auction)
{
    ...
    if (sWorld.getConfig(CONFIG_GM_LOG_TRADE))
    {
        ...
        else
        {
            ...
            bidder_security = bidder_accId ? AccountMgr::GetSecurity(bidder_accId) : SEC_PLAYER;
...
//          uncomment when will be implemented (if will be)
...
//            {
                ...
                {
                    case ALLIANCE: houseid = 1; break;//player->GetAuctionAccessMode() == 0 ? 1 : 6; break;
                    case HORDE:    houseid = 6; break;//player->GetAuctionAccessMode() == 0 ? 6 : 1; break;
                }
//            }
        }
    }
...
bool AuctionEntry::BuildAuctionInfo(WorldPacket & data) const
{
    ...
    data << uint32((expireTime-time(NULL))*IN_MILISECONDS); // time left
...
void AuctionEntry::SaveToDB() const
{
    ...
    stmt.addUInt32(itemRandomPropertyId);
