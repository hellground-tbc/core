...
std::string CreateDumpString(char const* tableName, QueryResultAutoPtr result)
{
...
void StoreGUID(QueryResultAutoPtr result,uint32 field,std::set<uint32>& guids)
{
    Field* fields = result->Fetch();
...
void StoreGUID(QueryResultAutoPtr result,uint32 data,uint32 field, std::set<uint32>& guids)
{
    Field* fields = result->Fetch();
...
void PlayerDumpWriter::DumpTable(std::string& dump, uint32 guid, char const*tableFrom, char const*tableTo, DumpTableType type)
{
    ...
    do
    {
        ...
        QueryResultAutoPtr result = CharacterDatabase.PQuery("SELECT * FROM %s WHERE %s", tableFrom, wherestr.c_str());
        if(!result)
            return;
...
DumpReturn PlayerDumpReader::LoadDump(const std::string& file, uint32 account, std::string name, uint32 guid)
{
    ...
    {
        QueryResultAutoPtr result = CharacterDatabase.PQuery("SELECT COUNT(guid) FROM characters WHERE account = '%d'", account);
        uint8 charcount = 0;
        if ( result )
        {
            Field *fields=result->Fetch();
