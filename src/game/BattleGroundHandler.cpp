...
void WorldSession::_HandleBattleGroundJoinOpcode(WorldPacket &recv_data)
{
    ...
    recv_data >> bg_type_id;                                // battleground type id (DBC id)
    recv_data >> instance_id;                               // instance id, 0 if First Available selected
    recv_data >> join_as_group;                             // join as group

    if(bg_type_id >= MAX_BATTLEGROUND_TYPES)
    {
    ...
    uint32 bg_queue_type_id = sBattleGroundMgr.BGQueueTypeId(bg_type_id, 0);
    ...
    BattleGround *bg = sBattleGroundMgr.GetBattleGroundTemplate(bg_type_id);
    ...
    if(!join_as_group)
    {
        ...
        {
            sBattleGroundMgr.BuildGroupJoinedBattlegroundPacket(&data, BG_JOIN_DESERTER_IN_GROUP);
        ...
        {
            sBattleGroundMgr.BuildGroupJoinedBattlegroundPacket(&data, BG_JOIN_ONLY_3_QUEUES);
            _player->GetSession()->SendPacket(&data);
            return;
        }

        // check if already in queue
        if (_player->GetBattleGroundQueueIndex(bg_queue_type_id) < PLAYER_MAX_BATTLEGROUND_QUEUES)
    ...
    else
    {
        ...
        uint32 err = group->CanJoinBattleGroundQueue(bg_type_id, bg_queue_type_id, 0, bg->GetMaxPlayersPerTeam(), false, 0);
        if (err != BG_JOIN_ERR_OK)
        {
            if (err == LANG_BG_GROUP_MEMBER_DESERTER)
            {
                sBattleGroundMgr.BuildGroupJoinedBattlegroundPacket(&data, BG_JOIN_DESERTER_IN_GROUP);
                _player->GetSession()->SendPacket(&data);
            }
    ...
    sBattleGroundMgr.m_BGQueues[bg_queue_type_id].AddGroup(_player, join_as_group, instance_id);
    sBattleGroundMgr.m_BGQueues[bg_queue_type_id].UpdateBGQueue(_player->GetBattleGroundQueueIdFromLevel());
}
...
// Enter BG
// Leave Queue
void WorldSession::_HandleBattleGroundPlayerPortOpcode(WorldPacket &recv_data)
{
    CHECK_PACKET_SIZE(recv_data, 1+1+4+2+1);

    uint8 arena_type;                                       // arenatype if arena
    uint8 unk2;                                             // unk, can be 0x0 (may be if was invited?) and 0x1
    uint32 instance_id;
    uint32 bg_type_id;                                      // type id from dbc
    uint16 unk;                                             // 0x1F90 constant?
    uint8 action;                                           // enter battle 0x1, leave queue 0x0

    recv_data >> arena_type >> unk2 >> bg_type_id >> unk >> action;

    sLog.outDebug( "WORLD: Recvd CMSG_BATTLEFIELD_PORT Message %u - %u", unk2, unk);
    ...
    if(bg_type_id >= MAX_BATTLEGROUND_TYPES)
    {
        ...
        if(_player->InBattleGroundQueue())
        {
            ...
            for (uint32 i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; i++)
            {
                ...
                _QueuedPlayersMap::iterator itr_player = sBattleGroundMgr.m_BGQueues[queue_id].m_queuedPlayers[_player->GetBattleGroundQueueIdFromLevel()].find(_player->GetGUID());
                // if the player is not in queue, contine
                if(itr_player == sBattleGroundMgr.m_BGQueues[queue_id].m_queuedPlayers[_player->GetBattleGroundQueueIdFromLevel()].end())
                    continue;

                // no group information, this should never happen
                if(!itr_player->second)
                    continue;

                BattleGround *bg = NULL;

                // get possibly needed data from groupinfo
                bg_type_id = itr_player->second->bg_type_id;
                //uint8 arenatype = itrPlayerStatus->second.group_info->ArenaType;
                //uint8 israted = itrPlayerStatus->second.group_info->IsRated;
                uint8 status = 0;


                if(!itr_player->second->bg_instance_GUID)
                {
                    // not invited to bg, get template
                    bg = sBattleGroundMgr.GetBattleGroundTemplate(bg_type_id);
                    status = STATUS_WAIT_QUEUE;
                }
                else
                {
                    // get the bg we're invited to
                    BattleGround * bg = sBattleGroundMgr.GetBattleGround(itr_player->second->bg_instance_GUID);
                ...
                WorldPacket data;
                sBattleGroundMgr.BuildBattleGroundStatusPacket(&data, bg, _player->GetTeam()/*itrPlayerStatus->second.GroupInfo->Team?itrPlayerStatus->second.GroupInfo->Team:_player->GetTeam()*/, i, status, INVITE_ACCEPT_WAIT_TIME, 0, 0,0);//arenatype, israted);
                SendPacket(&data);
            }
        }
        return;
    ...
    uint32 bg_queue_level_id = _player->GetBattleGroundQueueIdFromLevel();
    uint32 bg_queue_type_id = sBattleGroundMgr.BGQueueTypeId(bg_type_id, arena_type);;
    // get the bg what we were invited to

    _QueuedPlayersMap::iterator itr_player = sBattleGroundMgr.m_BGQueues[bg_queue_type_id].m_queuedPlayers[bg_queue_level_id].find(_player->GetGUID());
    if(itr_player == sBattleGroundMgr.m_BGQueues[bg_queue_type_id].m_queuedPlayers[bg_queue_level_id].end())
    {
        sLog.outError("Battleground: itrplayerstatus not found.");
        return;
    }
    instance_id = itr_player->second->bg_instance_GUID;

    // if action == 1, then instanceId is _required_
    if(!instance_id && action == 1)
    {
        sLog.outError("Battleground: instance not found.");
        return;
    }

    BattleGround *bg = sBattleGroundMgr.GetBattleGround(instance_id);
...
//        _QueuedPlayersMap::iterator pitr = sBattleGroundMgr.m_BGQueues[bgQueueTypeId].m_queuedPlayers[_player->GetBattleGroundQueueIdFromLevel()].find(_player->GetGUID());
//        if(pitr != sBattleGroundMgr.m_BGQueues[bgQueueTypeId].m_queuedPlayers[_player->GetBattleGroundQueueIdFromLevel()].end()
//            && pitr->second)

        if (itr_player->second)
        {
            team = itr_player->second->team;
            arenatype = itr_player->second->arena_type;
            israted = itr_player->second->is_rated;
            rating = itr_player->second->arena_team_rating;
            opponentsRating = 0;//pitr->second->groyp_info->OpponentsTeamRating;
        }
        else
        {
        ...
        WorldPacket data;
        enum BattleGroundPortAction
        {
            PORT_ACTION_LEAVE_QUEUE       = 0,
            PORT_ACTION_JOIN_BATTLEGROUND = 1
        };

        switch(action)
        {
            case PORT_ACTION_JOIN_BATTLEGROUND:         // port to battleground

                if(!_player->IsInvitedForBattleGroundQueueType(bg_queue_type_id))
                    return;                                     // cheating?
                ...
                queueSlot = _player->GetBattleGroundQueueIndex(bg_queue_type_id);
                ...
                sBattleGroundMgr.m_BGQueues[bg_queue_type_id].RemovePlayer(_player->GetGUID());//, false);
                ...
                sBattleGroundMgr.SendToBattleGround(_player, instance_id);
                // add only in HandleMoveWorldPortAck()
                // bg->AddPlayer(_player,team);
                sLog.outDebug("Battleground: player %s (%u) joined battle for bg %u, bgtype %u, queue type %u.",_player->GetName(),_player->GetGUIDLow(),bg->GetInstanceID(),bg->GetTypeID(),bg_queue_type_id);
                break;
            case PORT_ACTION_LEAVE_QUEUE:       // leave queue
                queueSlot = _player->GetBattleGroundQueueIndex(bg_queue_type_id);
                ...
                */
                ...
                _player->RemoveBattleGroundQueueId(bg_queue_type_id); // must be called this way, because if you move this call to queue->removeplayer, it causes bugs
                sBattleGroundMgr.BuildBattleGroundStatusPacket(&data, bg, _player->GetTeam(), queueSlot, STATUS_NONE, 0, 0);
                sBattleGroundMgr.m_BGQueues[bg_queue_type_id].RemovePlayer(_player->GetGUID());//, true);
                // player left queue, we should update it, maybe now his group fits in
                sBattleGroundMgr.m_BGQueues[bg_queue_type_id].UpdateBGQueue(_player->GetBattleGroundQueueIdFromLevel());
                SendPacket(&data);
                sLog.outDebug("Battleground: player %s (%u) left queue for bgtype %u, queue type %u.",_player->GetName(),_player->GetGUIDLow(),bg->GetTypeID(),bg_queue_type_id);
                break;
            default:
...
// rewrite
void WorldSession::_HandleBattleGroundArenaJoin(WorldPacket &recv_data)
{
    // not supported :D
    return;
    ...
    uint8 as_group;                                         // asGroup
    uint8 is_rated;                                         // isRated
    Group *group;

    recv_data >> guid >> type >> as_group >> is_rated;
    ...
    // if rated and player is already in queues send error
    if (is_rated && _player->InBattleGroundQueue())
    {
        // TODO: check other players from group and send proper error
        sBattleGroundMgr.BuildGroupJoinedBattlegroundPacket(&data, BG_JOIN_RATED_WHILE_OTHER);
        _player->GetSession()->SendPacket(&data);
        return;
    }
    ...
    else
    {
        ...
        if(!group)
            return;
        uint32 err = group->CanJoinBattleGroundQueue(bgTypeId, bgQueueTypeId, arenatype, arenatype, (bool)is_rated, type);
        if (err != BG_JOIN_ERR_OK)
        {
            SendBattleGroundOrArenaJoinError(err);
            return;
    ...
    if(1)// alwaysas_group)
    {
      //  sBattleGroundMgr.m_BGQueues[bgQueueTypeId].AddArenaGroup(_player, as_group, arenatype, is_rated, arenaRating);
      //  sBattleGroundMgr.m_BGQueues[bgQueueTypeId].Update(_player->GetBattleGroundQueueIdFromLevel());
       ...
       /* for(GroupReference *itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            ...
            WorldPacket data;
            // send status packet (in queue)
            sBattleGroundMgr.BuildBattleGroundStatusPacket(&data, bg, member->GetTeam(), queueSlot, STATUS_WAIT_QUEUE, 0, 0, arenatype, is_rated);
            member->GetSession()->SendPacket(&data);
            sBattleGroundMgr.BuildGroupJoinedBattlegroundPacket(&data, bgTypeId);
            member->GetSession()->SendPacket(&data);
            sBattleGroundMgr.m_BattleGroundQueues[bgQueueTypeId].AddPlayer(member, ginfo);
    ...
    else
    {
        ...
        WorldPacket data;
        // send status packet (in queue)
        sBattleGroundMgr.BuildBattleGroundStatusPacket(&data, bg, _player->GetTeam(), queueSlot, STATUS_WAIT_QUEUE, 0, 0, arenatype, is_rated);
        SendPacket(&data);
        GroupQueueInfo * ginfo = sBattleGroundMgr.m_BattleGroundQueues[bgQueueTypeId].AddGroup(_player, bgTypeId, arenatype, is_rated, arenaRating);
//        sBattleGroundMgr.m_BGQueues[bgQueueTypeId].AddPlayer(_player, ginfo);
  //      sBattleGroundMgr.m_BattleGroundQueues[bgQueueTypeId].Update(bgTypeId, _player->GetBattleGroundQueueIdFromLevel(), arenatype, is_rated, arenaRating);
