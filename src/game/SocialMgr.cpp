...
void SocialMgr::GetFriendInfo(Player *player, uint32 friendGUID, FriendInfo &friendInfo)
{
    ...
    bool gmInWhoList = sWorld.getConfig(CONFIG_GM_IN_WHO_LIST) || player->GetSession()->HasPermissions(PERM_GMT | PERM_HEAD_DEVELOPER);
    ...
    if (pFriend && pFriend->GetName() &&
        (player->GetSession()->HasPermissions(PERM_GMT | PERM_HEAD_DEVELOPER) ||
    ...
    for (SocialMap::iterator itr = m_socialMap.begin(); itr != m_socialMap.end(); ++itr)
    {
        ...
        if (itr2 != itr->second.m_playerSocialMap.end() && (itr2->second.Flags & SOCIAL_FLAG_FRIEND))
        {
            ...
            if (pFriend && pFriend->IsInWorld() &&
                (pFriend->GetSession()->HasPermissions(PERM_GMT | PERM_HEAD_DEVELOPER) ||
                (pFriend->GetTeam() == team || allowTwoSideWhoList) &&
