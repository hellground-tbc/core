    ...
    for (int i=0; i<TRADE_SLOT_TRADED_COUNT; i++)
    {
        ...
        if (traderCanTrade && playerCanTrade)
        {
            ...
            if (hisItems[i])
            {
                ...
                if (_player->pTrader->GetSession()->GetPermissions() & PERM_GMT && sWorld.getConfig(CONFIG_GM_LOG_TRADE))
                {
...
void WorldSession::HandleAcceptTradeOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    if (_player->pTrader->acceptTrade)
    {
        ...
        {
            if (_player->GetSession()->GetPermissions() & PERM_GMT && _player->tradeGold > 0)
            {
            ...
            if (_player->pTrader->GetSession()->GetPermissions() & PERM_GMT && _player->pTrader->tradeGold > 0)
            {
