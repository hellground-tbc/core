...
namespace LuaGuild
{
    ...
    {
        sEluna->Push(L, sObjectAccessor.FindPlayer(guild->GetLeader()));
        return 1;
    }
    ...
    {
        Player* player = sEluna->CHECKOBJ<Player>(L, 2);
        uint8 rankId = sEluna->CHECKVAL<uint8>(L, 3, 0);
...
#ifdef MANGOS
        guild->DelMember(player->GetGUID(), isDisbanding);
#else
    ...
    int ChangeMemberRank(lua_State* L, Guild* guild)
    {
        ...
        guild->ChangeRank(player->GetGUID(), newRank);
        return 0;
    }
