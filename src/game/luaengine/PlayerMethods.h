...
namespace LuaPlayer
{
    ...
    {
        sEluna->Push(L, player->CanUninviteFromGroup() == PARTY_RESULT_OK);
        return 1;
    }
    ...
    int UnbindInstance(lua_State* L, Player* player)
    {
        ...
        if (difficulty < TOTAL_DIFFICULTIES)
            player->UnbindInstance(map, (DungeonDifficulties)difficulty);
        return 0;
    }
    ...
    int SetSkill(lua_State* L, Player* player)
    {
        ...
        uint16 currVal = sEluna->CHECKVAL<uint16>(L, 3);
        uint16 maxVal = sEluna->CHECKVAL<uint16>(L, 4);

        player->SetSkill(id, currVal, maxVal);
        return 0;
    }
    ...
    int RemoveSpell(lua_State* L, Player* player)
    {
        ...
        player->removeSpell(entry, disabled);
        return 0;
    }
    ...
    int KilledMonsterCredit(lua_State* L, Player* player)
    {
        ...
        player->KilledMonster(entry, player->GetGUID());
        return 0;
    }
    ...
    int GetQuestLevel(lua_State* L, Player* player)
    {
        ...
        sEluna->Push(L, player->GetQuestOrPlayerLevel(quest));
        return 1;
    }
    ...
    int Whisper(lua_State* L, Player* player)
    {
        ...
        player->Whisper(text, lang, uint64(guid));
        return 0;
    }
    ...
    int GiveXP(lua_State* L, Player* player)
    {
        ...
        if (!pureXP)
        {
            if (victim)
            {
                ...
                Unit::AuraList const& ModXPPctAuras = player->GetAurasByType(SPELL_AURA_MOD_XP_PCT);
        ...
        player->SendLogXPGain(xp, victim, rested_bonus_xp, false);
    ...
    int EquipItem(lua_State* L, Player* player)
    {
        ...
        if (!item)
        {
            ...
            uint8 result = player->CanEquipItem(slot, dest, item, false);
    ...
    int CanEquipItem(lua_State* L, Player* player)
    {
        ...
        if (!item)
        {
            uint32 entry = sEluna->CHECKVAL<uint32>(L, 2);
            uint16 dest;
            uint8 msg = player->CanEquipNewItem(slot, dest, entry, false);
            if (msg != EQUIP_ERR_OK)
            {
                sEluna->Push(L, false);
                return 1;
        ...
        {
            uint16 dest;
            uint8 msg = player->CanEquipItem(slot, dest, item, false);
            if (msg != EQUIP_ERR_OK)
            {
                sEluna->Push(L, false);
                return 1;
    ...
    {
        sEluna->Push(L, player->GetMap()->GetUnit(player->GetSelection()));
        return 1;
    }

    int GetGMRank(lua_State* L, Player* player)
    {
        sEluna->Push(L, player->GetSession()->GetPermissions());
        return 1;
    }
    ...
    int AdvanceAllSkills(lua_State* L, Player* player)
    {
        ...
        static const uint32 skillsArray[] = { SKILL_BOWS, SKILL_CROSSBOWS, SKILL_DAGGERS, SKILL_DEFENSE, SKILL_UNARMED, SKILL_GUNS, SKILL_AXES, SKILL_MACES, SKILL_SWORDS, SKILL_POLEARMS,
            ...
            SKILL_ALCHEMY, SKILL_HERBALISM, SKILL_ENGINERING, SKILL_JEWELCRAFTING, SKILL_LEATHERWORKING, SKILL_LOCKPICKING, SKILL_SKINNING, SKILL_TAILORING,
        };
    ...
    int ResetTypeCooldowns(lua_State* L, Player* player)
    {
        ...
        bool update = sEluna->CHECKVAL<bool>(L, 3, true);
        player->RemoveSpellCooldown(category, update);
        return 0;
    }
