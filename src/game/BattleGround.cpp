...
{
    Creature *creature = GetBgMap()->GetCreature(m_BgObjects[type]);
    if(!creature)
...
bool BattleGround::DelCreature(uint32 type)
{
    ...
    Creature *cr = GetBgMap()->GetCreature(m_BgObjects[type]);
    if(!cr)
    {
...
void BattleGround::PlayerRelogin(uint64 guid)
{
    ...
    if(!plr)
    {
        sLog.outError("BattleGround: Player " UI64FMTD " not found!", guid);
        return;
    }
