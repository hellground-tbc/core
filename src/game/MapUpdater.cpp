    ...
    public:
        ...
        virtual int call(void)
        {
            ...
            catch(...)
            {
                ...
                sLog.outCrash("CRASH: mapid: %u, instanceid: %u", m_map.GetId(), m_map.GetInstanceId());
                sLog.outCrash("\r\n************ BackTrace *************\r\n%s\r\n***********************************\r\n", stackTrace.c_str());
...
void MapUpdater::FreezeDetect()
{
    ...
    if (!m_threads.empty())
    {
        for (ThreadMapMap::const_iterator itr = m_threads.begin(); itr != m_threads.end(); ++itr)
        {
            ...
            {
                sLog.outCrash("MapUpdater::FreezeDetect thread "I64FMT" possible freezed (is update map %u instance %u). Killing.",itr->first, itr->second.GetId(), itr->second.GetInstanceId());
