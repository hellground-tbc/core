...
bool Spell::CheckTarget(Unit* target, uint32 eff)
{
    ...
    {
        case 40869:
            if (target->HasAura(43690, 0))
                return false;
        break;
        case 43657:
            if (target->HasAura(44007, 0))
                return false;
        break;
        case 28062:     // Positive Charge
        case 39090:
            if (target->HasAura(28059, 0) || target->HasAura(39088, 0))
                return false;
        break;
        case 28085:     // Negative Charge
        case 39093:
            if (target->HasAura(28084, 0) || target->HasAura(39091, 0))
                return false;
        break;
        case 44869:     // Spectral Blast
            if (target->HasAura(44867, 0) || m_caster->getVictim() == target)
                return false;
        break;
        default:
            break;
